...
static void resize_queue( struct cku_queue *queue )
{
	...
	queue->v = xrealloc( queue->v, queue->n_elem, n_elem, elem_len );
	...
	while( j != 0 ) {
		...
		memcpy( tmp, vj , elem_len );
		memcpy( vj , vi , elem_len );
		memcpy( vi , tmp, elem_len );
...
{
	struct cku_queue *queue = xalloc( 1, sizeof(*queue) );
...
{
	return ( queue->i_begin == queue->i_end ? true : false );
}
